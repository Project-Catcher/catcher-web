...
interface RegexValidationType {
  [key: string]: boolean;
}

export const useRegex = () => {
  const [isValidate, setIsValidate] = useState<RegexValidationType>({});
  ...
  const checkIdValidation = useCallback((id: string) => {
  ...
  const checkPasswordValidation = useCallback(
    (newPassword: string, checkNewPassword: string) => {
      ...
      if (newPassword === checkNewPassword) {
        if (regexAll.test(newPassword))
          setIsValidate((prev) => ({ ...prev, password: true }));
        else if (regexNumSpecial.test(newPassword))
          setIsValidate((prev) => ({ ...prev, password: true }));
        else if (regexEnglishSpecial.test(newPassword))
          setIsValidate((prev) => ({ ...prev, password: true }));
        else if (regexEnglishNum.test(newPassword))
          setIsValidate((prev) => ({ ...prev, password: true }));
